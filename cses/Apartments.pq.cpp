...
int main(){
  int n, m, k, x;
  cin >> n >> m >> k;
  map<int, VI> m2a; // man can match apartment
  map<int, VI> a2m; // apartment match man
  map<int, int> am_cnt;
  map<int, int> ma_cnt;  
  priority_queue<node> Qm;
  ...
  return 0;
  REP(i, n)
    if (m2a.find(i) != m2a.end())
      Qm.push(node(m2a[i].size(), i));
  
  //  REP(j, m) Qa.push(node(a2m[j].size(), j));
  int ans = 0;
  while (Qm.size()){

    while(used_man[Qm.top().id]) Qm.pop();
    int min_i = Qm.top().id;
    ...
    REP(j, SIZE(a2m[min_j])){
      int this_i = a2m[min_j][j];
      ma_cnt[this_i]--;
      if (!used_man[this_i] && ma_cnt[this_i])
        Qm.push(node(ma_cnt[this_i], this_i));
    }
